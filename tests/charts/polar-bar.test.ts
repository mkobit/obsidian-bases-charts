...
describe(
	...
	() => {
		...
		it(
			'should generate a basic polar bar chart',
			() => {
				...
				expect(angleAxis.data).toEqual(['A',
					'B']);
				...
				expect(barSeries.encode).toEqual({ angle: 'x',
					radius: 'y' });
			},
		);

		it(
			'should support series grouping',
			() => {
				const option = createPolarBarChartOption(
					data,
					'angle',
		...
		it(
			'should support stacking with grouping',
			() => {
				const option = createPolarBarChartOption(
					data,
					'angle',
