...
describe('createBulletChartOption', () => {
    const data = [
        { category: 'A', value: 10, target: 12, low: 5, mid: 15, high: 20 },
        { category: 'B', value: 20, target: 18, low: 10, mid: 25, high: 30 },
        { category: 'C', value: 15 }, // Missing target and ranges
    ...
    it('should create range series when range props are provided', () => {
        const option = createBulletChartOption(data, 'category', 'value', {
            targetProp: 'target',
            rangeLowProp: 'low',
            rangeMidProp: 'mid',
            rangeHighProp: 'high'
    ...
    it('should calculate range deltas correctly', () => {
        const option = createBulletChartOption(data, 'category', 'value', {
            rangeLowProp: 'low',
            rangeMidProp: 'mid',
            rangeHighProp: 'high'
    ...
    it('should handle flipped axis with ranges', () => {
        const option = createBulletChartOption(data, 'category', 'value', {
            targetProp: 'target',
            flipAxis: true,
            rangeLowProp: 'low'
        ...
        expect(target.symbolSize).toEqual([4, 30]); // Horizontal bar, vertical marker
    });
});
