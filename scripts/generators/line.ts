...
export const lineChartArbitrary = fc.record({
    start: fc.integer({ min: 0, max: 1000000000 }), // Offset from epoch in seconds
    count: fc.integer({ min: 5, max: 20 }),
    values: fc.array(fc.integer({ min: 0, max: 100 }), { minLength: 5, maxLength: 20 })
}).chain(input => {
    // Ensure values match count
    const values = input.values.slice(0, input.count);
    return fc.constant({
        type: 'line',
