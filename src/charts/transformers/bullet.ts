...
export function createBulletChartOption(
	...
	const normalizedData = R.map(
		...
		(item) => {
			...
			const targetVal = targetProp ? Number(getNestedValue(
				item,
				targetProp,
			)) : undefined;

			// Range Values
			const r1Raw = (rangeLowProp !== undefined) ? Number(getNestedValue(
				item,
				rangeLowProp,
			)) : 0;
			const r2Raw = (rangeMidProp !== undefined) ? Number(getNestedValue(
				item,
				rangeMidProp,
			)) : 0;
			const r3Raw = (rangeHighProp !== undefined) ? Number(getNestedValue(
				item,
				rangeHighProp,
			)) : 0;
