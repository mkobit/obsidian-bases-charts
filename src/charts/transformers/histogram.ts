...
import { BasesData, BaseTransformerOptions } from './base';
...
export function createHistogramChartOption(
    ...
    const values = R.pipe(
        data,
        R.map(row => getNestedValue(row, valueProp)),
    ...
    // 2. Calculate Min/Max
    const sortedValues = R.sortBy(values, v => v);
    ...
    const defaultBinCount = Math.max(1, options?.binCount ?? sturges);

    // Use ternary instead of if block
    const hasBinWidth = (options?.binWidth ?? 0) > 0;
    const binCountFromWidth = hasBinWidth
        ? Math.max(1, Math.ceil(safeRange / (options?.binWidth ?? 1)))
        : defaultBinCount;
    ...
    const xAxisData = R.map(bins, b => b.label);
    const seriesData = R.map(bins, b => b.count);
